...
>
> **Semantics of the resolution value in a scene header [PARAM]:** the number above is a **suggested default**, and the MP/resolution written into a `Technical settings` header matters **only when the workflow is submitted to the queue via comfy-mcp** (it is the queued run's requested size). When the user **copy/pastes the prompt manually into ComfyUI**, the resolution is set in a **dedicated node of the workflow** — the narrative prompt body does not carry it — so a fixed value baked into the header does not govern that render. The MP/resolution ladder values seen in downloaded templates (Civitai, HuggingFace, tutorials, YouTube) are **reference only**; the user decides the tier in the questionnaire, per their preference. Always record the chosen tier in the technical settings header for the comfy-mcp path (see §2.6).
...
- **Resolution [PARAM]:** ① 0.7 MP 1152×640 (⭐ — short film, upscaled in post); ② 0.9 MP 1280×736; ③ 0.98 MP 1344×768 (official 768p); ④ small-screen 0.5 MP. Ask which tier the user prefers (preference question); the chosen value goes into the scene header **only for the comfy-mcp/queue path** — on a manual copy/paste into ComfyUI the resolution is set in the workflow's own node, not by the header (see §1.1 Semantics).
